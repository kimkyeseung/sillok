...
export function ReplyActions({ threadId, replyId, likeCount }: ReplyActionsProps) {
  const { user } = useAuth();
  const router = useRouter();
  ...
  return (
    <div className="pl-[38px]">
      <div className="mt-1 flex items-center gap-1">
        ...
        <button
          type="button"
          onClick={() => (user ? setReplying((v) => !v) : router.push('/login'))}
          className="flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-gray-400 transition-colors hover:bg-gray-50 hover:text-gray-600"
        >
      ...
      {replying && (
        <div className="mt-2">
          <ReplyForm threadId={threadId} parentId={replyId} onSuccess={() => setReplying(false)} />
        </div>
      )}
    </div>
  );
